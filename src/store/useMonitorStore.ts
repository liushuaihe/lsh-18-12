...
import { MOCK_DEVICES, MOCK_SENSORS, MOCK_CALIBRATIONS } from '../data/mockData';
import {
  canDeviceStart,
  getDependencyOrder,
...
interface MonitorStore {
  ...
  addCalibration: (cal: Omit<Calibration, 'id'>) => void;
  updateCalibration: (id: string, updates: Partial<Calibration>) => void;
  deleteCalibration: (id: string) => void;
  getActiveCalibration: (sensorId: string) => Calibration | null;
  getCalibrationStatus: (sensorId: string) => CalibrationStatus;
  getCalibratedTemp: (sensorId: string) => number;
  getCalibratedHistory: (sensorId: string) => Array<{ t: number; v: number }>;
}
...
export const useMonitorStore = create<MonitorStore>((set, get) => ({
  ...
  getActiveCalibration: (sensorId) => {
    const state = get();
    const now = Date.now();
    const sensorCals = Object.values(state.calibrations)
      .filter(c => c.sensorId === sensorId && c.effectiveDate <= now && c.expiryDate >= now)
      .sort((a, b) => b.effectiveDate - a.effectiveDate);
    return sensorCals[0] || null;
  },

  getCalibrationStatus: (sensorId): CalibrationStatus => {
    const state = get();
    const now = Date.now();
    const UPCOMING_DAYS = 7 * 24 * 60 * 60 * 1000;

    const active = state.getActiveCalibration(sensorId);
    if (active) {
      if (active.expiryDate - now <= UPCOMING_DAYS) return 'upcoming';
      return 'valid';
    }

    const expired = Object.values(state.calibrations)
      .filter(c => c.sensorId === sensorId && c.expiryDate < now)
  ...
  getCalibratedTemp: (sensorId) => {
    const state = get();
    const sensor = state.sensors[sensorId];
    if (!sensor) return 0;
    const cal = state.getActiveCalibration(sensorId);
    if (!cal) return sensor.currentTemp;
    return sensor.currentTemp + cal.offset;
  },

  getCalibratedHistory: (sensorId) => {
    const state = get();
    const sensor = state.sensors[sensorId];
    if (!sensor) return [];
    const cal = state.getActiveCalibration(sensorId);
    if (!cal) return sensor.tempHistory;
    return sensor.tempHistory.map(p => ({ ...p, v: p.v + cal.offset }));
  },
}));
