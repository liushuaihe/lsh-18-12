...
const EXPIRED_COLOR = '#FF2D55';

export const TempChart: React.FC<Props> = ({ sensors, height = 220, windowMs = 60_000, showCalibrated = false }) => {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (const s of currSensors) {
        ...
        const history = showCalibrated && s.calibratedHistory ? s.calibratedHistory : sensor.tempHistory;
        const current = showCalibrated && s.calibratedTemp != null ? s.calibratedTemp : sensor.currentTemp;
      ...
      // Plot each sensor
      for (const s of currSensors) {
        const { sensor, color, calibrationExpired } = s;
        const history = showCalibrated && s.calibratedHistory ? s.calibratedHistory : sensor.tempHistory;
        const current = showCalibrated && s.calibratedTemp != null ? s.calibratedTemp : sensor.currentTemp;

        if (!history.length && current == null) continue;

        let lineColor = COLORS_BY_LEVEL[sensor.alertLevel] || color;
        if (calibrationExpired) {
          lineColor = EXPIRED_COLOR;
        }
        ...
        ctx.shadowBlur = calibrationExpired ? 12 : 6;
        ctx.beginPath();
        ctx.moveTo(x(allPoints[0].t), y(allPoints[0].v));
        for (let i = 1; i < allPoints.length; i++) {
          const p = allPoints[i];
        ...
        ctx.fillStyle = '#fff';
        ctx.shadowColor = lineColor;
        ctx.shadowBlur = calibrationExpired ? 15 : 10;
        ctx.beginPath();
        ...
        if (calibrationExpired) {
          ...
          ctx.setLineDash([3, 3]);
          ctx.beginPath();
          ctx.arc(ex, ey, 9 + pulse * 2, 0, Math.PI * 2);
          ctx.stroke();
          ctx.setLineDash([]);
        }
        ...
        ctx.shadowBlur = 4;
        ctx.fillText(`${current.toFixed(1)}°`, ex + 6, ey);
        ctx.shadowBlur = 0;

        if (calibrationExpired) {
          ctx.fillStyle = EXPIRED_COLOR;
          ctx.font = '9px JetBrains Mono, monospace';
          ctx.textAlign = 'left';
          ctx.textBaseline = 'top';
          ctx.fillText('已过期', ex + 6, ey + 8);
        }
      }
  ...
  }, [height, windowMs, showCalibrated]);
